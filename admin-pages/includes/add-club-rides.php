...
// Fetch existing rides_form for the club
$rides_gform_ids = $wpdb->get_var($wpdb->prepare(
    "SELECT rides_form FROM {$wpdb->prefix}clubs WHERE club_id = %d",
    $club_id
));

// Convert stored IDs to an array
$selected_gform_ids = $rides_gform_ids ? explode(',', $rides_gform_ids) : [];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_rides_gform'])) {
    $selected_gform_ids = isset($_POST['rides_gform_ids']) ? array_map('intval', $_POST['rides_gform_ids']) : [];

    // Update rides_form column in wp_clubs (store IDs as comma-separated string)
    $wpdb->update(
        "{$wpdb->prefix}clubs",
        ['rides_form' => implode(',', $selected_gform_ids)],
        ['club_id' => $club_id],
        ['%s'],
        ['%d']
    );
...
<?php if (isset($_GET['updated']) && $_GET['updated'] == true): ?>
    <div class="notice notice-success is-dismissible">
        <p><?php _e('Rides form IDs saved successfully!', 'textdomain'); ?></p>
    </div>
<?php endif; ?>
...
<form method="POST" action="">
    <table class="form-table">
        ...
        <tr>
            <th scope="row">
                <label for="rides_gform_ids"><?php _e('Select Gravity Forms for Rides', 'textdomain'); ?></label>
            </th>
            <td>
                <select id="rides_gform_ids" name="rides_gform_ids[]" multiple style="width: 100%; height: 200px;">
    ...
    <input type="hidden" name="save_rides_gform" value="1" />
    <p class="submit">
