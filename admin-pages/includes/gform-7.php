...
add_action('init', 'gform_id_7_meta_handler_init');

function gform_id_7_meta_handler_init() {
    global $wpdb;

    // Pre-populate Gravity Form fields for Form ID: 7
    if (is_user_logged_in()) {
        add_filter('gform_pre_render_7', 'pre_populate_gform_7_fields');
    }

    function pre_populate_gform_7_fields($form) {
        global $wpdb;
    ...
    add_action('gform_after_submission_7', 'save_gform_7_fields', 10, 2);

    function save_gform_7_fields($entry, $form) {
        global $wpdb;
