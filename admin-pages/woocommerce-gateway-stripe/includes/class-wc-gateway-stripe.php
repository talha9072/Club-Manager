...
class WC_Gateway_Stripe extends WC_Stripe_Payment_Gateway {
	...
	public function __construct() {
		...
		$this->method_description = sprintf(
			__( 'Stripe works by adding payment fields on the checkout and then sending the details to Stripe for verification. <a href="%1$s" target="_blank">Sign up</a> for a Stripe account, and <a href="%2$s" target="_blank">get your Stripe account keys</a>.', 'woocommerce-gateway-stripe' ),
		...
		// Try to load dynamic club-based Stripe keys
		$club_id = null;
		...
		if ( $club_id && strtolower($club_id) !== 'global' ) {
			...
			$club_data = $wpdb->get_row( $wpdb->prepare(
				"SELECT stripe_testmode_enabled, stripe_test_secret_key, stripe_test_publishable_key, stripe_live_secret_key, stripe_live_publishable_key FROM {$wpdb->prefix}clubs WHERE club_id = %d",
				$club_id
			) );

			if ( $club_data ) {
				if ( $club_data->stripe_testmode_enabled ) {
					$this->secret_key = $club_data->stripe_test_secret_key;
					$this->publishable_key = $club_data->stripe_test_publishable_key;
					$this->testmode = true;
				} else {
					$this->secret_key = $club_data->stripe_live_secret_key;
					$this->publishable_key = $club_data->stripe_live_publishable_key;
					$this->testmode = false;
				}

				WC_Stripe_API::set_secret_key( $this->secret_key );
				WC_Stripe_Logger::log( "Club-based Stripe keys loaded for Club ID {$club_id} — Mode: " . ( $this->testmode ? 'Test' : 'Live' ) );
			}
		}

		// Now load plugin settings, but don't override dynamic keys if set
		...
		// Only assign fallback keys if dynamic keys were not already set
		...
		public function is_available() {
			...
			if ( $club_id && strtolower($club_id) !== 'global' ) {
				...
				if ( $club ) {
					$has_keys = !empty($club->stripe_test_secret_key) || !empty($club->stripe_live_secret_key);
					if ( $has_keys ) {
						return true;
					}
				} else {
			...
			// If no club_id or it's global, fallback to plugin config
